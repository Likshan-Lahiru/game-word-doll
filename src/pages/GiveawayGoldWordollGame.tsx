                ...
                <div className="flex flex-col h-dvh min-h-dvh pb-[env(safe-area-inset-bottom)]">
                    <div className="relative flex flex-col h-full">
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center overflow-hidden px-4">
                            {/* Hidden input: keep inert (no OS keyboard) */}
                            <input
                                ref={inputRef}
                                type="text"
                                inputMode="none"
                            ...
                            {/* Last attempt */}
                            <div className="flex justify-center mt-[clamp(0.5rem,5vh,1.25rem)] mb-2">
                                <div
                                    className={`grid grid-cols-1 ${S.gridGap} text-2xl font-[Inter]`}
                                            ...
                                            className={`${S.tileBoxLg} flex items-center justify-center ${
                                                currentAttempt[index]
                                                    ? lockedPositions[index]
                                                        ...
                                                        : 'bg-gray-700'
                                                    : 'bg-[#374151]'
                                            } rounded-md text-white font-bold shadow-md font-[Inter] ${
                        ...
                        {/* Mobile keyboard */}
                        <div className="flex-none pb-2 mb-4">
                                    ...
                                    >
                                        {row.map((key, keyIndex) => (
                                            <button
                                                ...
                                                className={`${key === 'ENTER' || key === 'Backspace' ? 'w-[60px]' : 'w-[32px]'} ${S.key} m-[clamp(0.10rem,0.10vw,0.3rem)] rounded-md bg-[#67768f] hover:bg-[#5a697f] text-white font-bold ${
                                                    key === 'ENTER' ? 'text-xs' : 'text-md'
                    ...
                    {!isAuthenticated ? (
                        <>
                            <WinModal isOpen={showWinModal} onClose={() => setShowWinModal(false)} reward={winAmount} gameType="wordoll" />
                            <LoseModal isOpen={showLoseModal} onClose={() => setShowLoseModal(false)} penalty={betAmount} gameType="wordoll" />
                            <NoAttemptsModal isOpen={showNoAttemptsModal} onClose={() => setShowNoAttemptsModal(false)} penalty={betAmount} />
                        </>
                    ) : (
                        <>
                            <AuthenticatedWinModal isOpen={showWinModal} onClose={() => setShowWinModal(false)} reward={winAmount} />
                            <AuthenticatedLoseModal isOpen={showLoseModal} onClose={() => setShowLoseModal(false)} penalty={betAmount} />
                            <AuthenticatedNoAttemptsModal isOpen={showNoAttemptsModal} onClose={() => setShowNoAttemptsModal(false)} penalty={betAmount} />
                        </>
                    )}
                </div>
            </div>
    ...
    return (
        ...
        >
            <div className="relative flex flex-col h-full">
                ...
                <div className="flex-none pb-6 mb-10">
                    <div className="w-full max-w-md mx-auto">
                        {mobileKeyboard.map((row, rowIndex) => (
                            <div
                                key={rowIndex}
