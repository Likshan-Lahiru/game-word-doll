...
export function GiveawayEntry() {
  ...
  const CustomWordollCard = () => {
    return <div className="h-full relative" onClick={e => e.stopPropagation()}>
        {isMobile ? <div className="rounded-xl overflow-hidden flex flex-col h-full relative">
            {/* Full image background */}
            <div className="absolute inset-0">
              <img src="/wordoll.png" alt="Wordoll" className="w-full h-full object-cover" />
            </div>
            {/* Title at the bottom */}
            <div className="mt-auto py-3 px-4 text-center bg-black/50 relative z-10">
              <h3 className="text-2xl font-bold text-white">Wordoll</h3>
            </div>
            {/* Overlay to capture clicks */}
            <div className="absolute inset-0 cursor-pointer" onClick={() => navigate('/giveaway-game')} />
          </div> : <>
            <WordollCard />
            {/* Overlay to capture the play button click */}
            <div className="absolute bottom-0 left-0 right-0 h-24 cursor-pointer" onClick={() => navigate('/giveaway-game')} />
          </>}
      </div>;
  };
  const CustomLockPickrCard = () => {
    return <div className="h-full relative" onClick={e => e.stopPropagation()}>
        {isMobile ? <div className="rounded-xl overflow-hidden flex flex-col h-full relative">
            {/* Full image background */}
            <div className="absolute inset-0">
              <img src="/lockpickr.png" alt="Lock Pickr" className="w-full h-full object-cover" />
            </div>
            {/* Title at the bottom */}
            <div className="mt-auto py-3 px-4 text-center bg-black/50 relative z-10">
  ...
  return <div className="flex flex-col w-full min-h-screen bg-[#1F2937] text-white">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-4 mb-auto">
          <div className={`flex flex-wrap justify-center ${isMobile ? 'gap-3' : 'gap-4'} w-full max-w-2xl mx-auto`}>
            {/* Equal height cards with custom navigation */}
            <div className={`${isMobile ? 'w-[48%]' : 'w-[220px]'} ${isMobile ? 'h-[250px]' : 'h-[380px]'}`}>
              <CustomWordollCard />
            </div>
            <div className={`${isMobile ? 'w-[48%]' : 'w-[220px]'} ${isMobile ? 'h-[250px]' : 'h-[380px]'}`}>
              <CustomLockPickrCard />
        ...
        <div className="w-full px-4 mb-8 mt-auto">
          <button className="bg-blue-500 hover:bg-blue-600 text-white py-3 px-8 rounded-full w-full max-w-md mx-auto block" onClick={() => navigate('/giveaway-game')} disabled={spinBalance <= 0}>
