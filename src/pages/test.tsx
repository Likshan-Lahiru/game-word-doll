...
export function WordollGame() {
    ...
    const [showInvalidWordMessage, setShowInvalidWordMessage] = useState(false)
    useEffect(() => {
    ...
    const checkGuess = useCallback(() => {
        ...
        const isWordInList = WORDS.includes(guess)
        if (!isWordInList) {
            setShowInvalidWordMessage(true)
            // Hide the message after 2 seconds
            setTimeout(() => {
                setShowInvalidWordMessage(false)
            }, 2000)
            return
        }
        // Save the current attempt as the last attempt before updating
    ...
    return (
        ...
        >
            ...
            {/* Not in word list message */}
            {showInvalidWordMessage && (
                <div className="bg-[#374151] text-center py-3 px-6 rounded-xl mb-4 mx-auto max-w-xs">
                    <p className="text-white text-lg font-medium">Not in word list</p>
                </div>
            )}
