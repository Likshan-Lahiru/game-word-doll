...
export function GemGameModePage() {
    ...
    const [displayLink, setDisplayLink] = useState('www.xyz.com')
    ...
    // Set balance selector to 'ticket' by default when component mounts
    useEffect(() => {
    ...
    const generateInviteLink = () => {
        try {
            ...
            if (!userId) return
            // Get the base URL (domain) of the current page
            const baseUrl = window.location.origin
            // Create the full invite link with userId and gameType parameters
            const fullLink = `${baseUrl}/gem-game-mode/?inviterId=${userId}&gameType=${gameType}`
            // Set the full link and a simplified display version
            setFullInviteLink(fullLink)
            setDisplayLink(new URL(baseUrl).hostname)
        } catch (error) {
    ...
    // Sync gameType from the query string on initial load (no UI changes)
    useEffect(() => {
        const params = new URLSearchParams(location.search)
        const fromQuery = params.get('gameType')?.toLowerCase()
        // only accept the two valid values
        const valid =
            fromQuery === 'lockpickr'
                ? 'lockpickr'
                : fromQuery === 'wordoll'
                    ? 'wordoll'
                    : null
        // If valid and different from what's already in route state, replace state
                    ...
                    {showNotEnoughPlayersModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
                            <div className="bg-[#374151] rounded-xl p-6 m-4 max-w-sm">
                                ...
                                <p className="mb-6 text-center">
                                    The game requires at least 2 players to start. Your Entries
                                    has been returned.
                                </p>
