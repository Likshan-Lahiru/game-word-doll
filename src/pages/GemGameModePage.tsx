        ...
        <div className="flex flex-col w-full min-h-screen bg-[#1F2937] text-white">
            ...
            <div className="flex-1 flex flex-col items-center justify-start pt-5 px-4">
                ...
                <div className="w-full max-w-md flex justify-between mb-5">
                    <button
                        onClick={() => setSelectedTicketAmount(0.2)}
                        className={`flex-1 py-1 px-2 h-14 bg-[#374151] rounded-2xl flex items-center justify-center mx-1 relative ${selectedTicketAmount === 0.2 ? 'font-bold' : ''}`}
                    >
                        <img
                        ...
                        />
                        <span className="text-xl font-[Inter]">0.20</span>
                        {selectedTicketAmount === 0.2 && (
                            <div className="absolute bottom-0 left-0 right-0 h-1 bg-white rounded-full"></div>
                        )}
                    </button>
                    <button
                        onClick={() => setSelectedTicketAmount(1)}
                        className={`flex-1 py-4 h-14 px-2 bg-[#374151] rounded-2xl flex items-center justify-center mx-1 relative ${selectedTicketAmount === 1 ? 'font-bold' : ''}`}
                    >
                        <img
                            src="https://uploadthingy.s3.us-west-1.amazonaws.com/65WCbcmf6dyyeqvjSAJHyp/fire.png"
                            alt="Ticket"
                            className="w-6 h-6 mr-2"
                        />
                        <span className="text-xl font-[Inter]">1</span>
                        {selectedTicketAmount === 1 && (
                            <div className="absolute bottom-0 left-0 right-0 h-1 bg-white rounded-full"></div>
                        )}
                    </button>
                    <button
                        onClick={() => setSelectedTicketAmount(9)}
                        className={`flex-1 py-4 h-14 px-2 bg-[#374151] rounded-2xl flex items-center justify-center mx-1 relative ${selectedTicketAmount === 9 ? 'font-bold' : ''}`}
                    >
                        <img
                            src="https://uploadthingy.s3.us-west-1.amazonaws.com/65WCbcmf6dyyeqvjSAJHyp/fire.png"
                            alt="Ticket"
                            className="w-6 h-6 mr-2"
                        />
                        <span className="text-xl font-[Inter]">9</span>
                        {selectedTicketAmount === 9 && (
                            <div className="absolute bottom-0 left-0 right-0 h-1 bg-white rounded-full"></div>
                        )}
                    </button>
                </div>
