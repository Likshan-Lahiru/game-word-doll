...
export function GemGameModePage() {
    ...
    // New state for not enough players modal
    ...
    *!/
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setCountdown((prevCountdown) => {
                if (prevCountdown <= 1) {
                    clearInterval(timer)
                    // When countdown reaches zero, check if there are enough players
                    if (hasJoined && groupSessionId) {
                        if (playersJoined < 2) {
                            // Not enough players, automatically leave the session
                            handleNotEnoughPlayers()
                        } else {
                            // Enough players, navigate to game
    ...
    // New function to handle not enough players
    const handleNotEnoughPlayers = async () => {
        try {
            setIsLeaving(true)
            ...
            // Show the not enough players modal
            setShowNotEnoughPlayersModal(true)
        } catch (err) {
            console.error('Error leaving game due to not enough players:', err)
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col w-full h-screen bg-[#1F2937] text-white items-center justify-center overflow-hidden">
    ...
    if (isMobile) {
        return (
            <div className="flex flex-col w-full h-screen max-h-screen bg-[#1F2937] text-white overflow-hidden">
                {/!* Back button *!/}
                ...
                {/!* Main Content *!/}
                <div className="flex-1 flex flex-col items-center justify-start pt-5 px-4 overflow-y-auto">
                    ...
                    {showNotEnoughPlayersModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
                            <div className="bg-[#374151] rounded-xl p-6 m-4 max-w-sm">
                                ...
                                <p className="mb-6 text-center">
                                    The game requires at least 2 players to start. Your Entries has
                                    been returned.
                                </p>
                    ...
                    <div className="w-full max-w-md bg-[#374151] rounded-xl p-2 text-center mb-20">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <button className="bg-[#2D7FF0] hover:bg-blue-600 px-3 py-1 mr-20 rounded-full text-xs font-[Inter]">
                                {' '}
                                Copy
                            </button>
                        </div>
    ...
    return (
        <div className="flex flex-col w-full h-screen max-h-screen bg-[#1F2937] text-white overflow-hidden">
            ...
            {showNotEnoughPlayersModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
                    <div className="bg-[#374151] rounded-xl p-8 max-w-md">
                        ...
                        <div className="flex justify-center">
                            <button
                                className="bg-[#3B82F6] hover:bg-blue-600 text-white font-semibold px-8 py-3 rounded-lg"
                                onClick={() => setShowNotEnoughPlayersModal(false)}
                            >
                                OK
                            </button>
            ...
            {/!* Main Content *!/}
            <div className="flex-1 flex flex-col items-center justify-start pt-0 px-4 overflow-y-auto">
                {/!* Ticket Selector Component *!/}
                <TicketSelector
                    options={ticketOptions}
                    selectedValue={selectedTicketAmount}
...
export function GemGameModePage() {
    ...
    // Set balance selector to ticket when coming from invite link
    useEffect(() => {
        // Check if the user arrived via an invite link by checking URL parameters
        const params = new URLSearchParams(location.search)
        const inviterId = params.get('gameType')
        // If inviterId exists in the URL, this is an invite link navigation
        if (inviterId) {
            // Set balance selector to ticket
            setSelectedBalanceType('ticket')
        }
    }, [location.search, setSelectedBalanceType])
    ...
    const generateInviteLink = () => {
        ...
        // Create a link with gameType and userId as parameters
        const link = `${baseUrl}/gem-game-mode?gameType=${gameType}`
        setInviteLink(link)
    }
    ...
    // Sync gameType from query string on initial load (no UI changes)
    useEffect(() => {
        const params = new URLSearchParams(location.search)
        const fromQuery = params.get('gameType')?.toLowerCase()
        // allow only the two valid values
        const valid =
            fromQuery === 'lockpickr'
                ? 'lockpickr'
                : fromQuery === 'wordoll'
                    ? 'wordoll'
                    : null
        // if valid and different from what's already in state, replace state
        if (valid && ((location.state as any)?.gameType ?? 'wordoll') !== valid) {
        ...
        return (
            <div className="flex flex-col w-full h-screen max-h-screen bg-[#1F2937] text-white overflow-hidden">
                ...
                <div className="flex-1 flex flex-col items-center justify-start pt-5 px-4 overflow-y-auto">
                    ...
                    {showNotEnoughPlayersModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
                            <div className="bg-[#374151] rounded-xl p-6 m-4 max-w-sm">
                                ...
                                <p className="mb-6 text-center">
                                    The game requires at least 2 players to start. Your ticket has
                                    been returned.
                                </p>
                                <div className="flex justify-center">
                                    <button
                    ...
                    {/* Invite Card - Updated with real link and copy functionality */}
                    <div className="w-full max-w-md bg-[#374151] rounded-xl p-6 text-center mb-3">
                        ...
                        <div className="flex items-center justify-between px-8">
                            <p className="text-lg text-[#006CB9] font-[Inter] truncate max-w-[60%]">
                                {window.location.origin}
                            </p>
                            <button
                                className="bg-[#2D7FF0] hover:bg-blue-600 px-3 py-1 rounded-full text-xs font-[Inter]"
            ...
            <div className="flex-1 flex flex-col items-center justify-start pt-0 px-4 overflow-y-auto">
                ...
                <div className="w-full max-w-md bg-[#374151] rounded-xl p-6 text-center mb-3">
                    ...
                    <div className="flex items-center justify-between px-16">
                        <p className="text-lg text-[#006CB9] font-[Inter] truncate  max-w-[100%]">
                            {window.location.origin}
                        </p>
                        <button
                            className="bg-[#2D7FF0] hover:bg-blue-600 px-3 py-1 rounded-full text-xs font-[Inter]"
