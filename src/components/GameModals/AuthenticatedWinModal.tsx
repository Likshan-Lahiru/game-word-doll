...
export function AuthenticatedWinModal({
    ...
    const { addCoins, temporaryVoucherBalance, temporaryTicketBalance, temporaryCoinBalance, setVoucherBalance , setTicketBalance, voucherBalance, ticketBalance, selectedBalanceType } = useGlobalContext()
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1F2937]">
            <div className="bg-[#374151] rounded-2xl p-6 text-center text-white w-[320px] flex flex-col items-center">
                <h2 className={`${selectedBalanceType !== 'ticket' ? 'mb-24' : 'mb-10'} text-3xl text-[#FFFFFF] font-semibold mt-10 font-[Inter]`}>Congratulations !</h2>
                <p className="text-[18px] font-medium mb-10 text-[#FFFFFF]">You Win</p>
                <div className={`${selectedBalanceType !== 'ticket' && 'mb-36' } flex items-center justify-center text-5xl font-bold`}>
                    <img
                    ...
                    { selectedBalanceType === 'ticket' ?
                        <span className="text-2xl font-medium font-[Inter] text-[#FFFFFF]">{temporaryCoinBalance.toLocaleString()}</span>
                        :
